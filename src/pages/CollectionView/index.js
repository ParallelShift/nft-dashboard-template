...
export default function CollectionView({light, vibrant, dark}) {
  ...
  const handleGraph = async(filter) => {
    ...
    // Request for floor prices and add parameters to format for graph
      try{
        const resp = await axios.get(api_call, {auth: {username: 'ckey_6fe0c838e7454ca8b9f0278afbf'}})
  ...
  const handleCollection = async() => {
    let collection = []
    try{
      const resp = await axios.get(`https://api.covalenthq.com/v1/${blockchain_id}/nft_market/collection/${address_id}/`,{auth: {username: 'ckey_6fe0c838e7454ca8b9f0278afbf'}})
